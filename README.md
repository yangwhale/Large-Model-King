...
| 神兽 | 大模型 | 有灵性的活物 | GPT、Claude、Gemini、Llama——每个门派的镇派之宝 |
| 灵核 | GPU/TPU 芯片 | 锻造智元的核心器官 | NVIDIA 的叫教廷灵核，Google 的叫道核 |
| 洞天 | 数据中心 | 完整的修炼空间 | 灵核、灵池、经脉、灵气俱全的自给自足之地 |
| 育兽法阵 | PyTorch/JAX | 孵化神兽的法阵 | 没有法阵，有再多灵核和智元也养不出神兽 |

### 洞天——修炼的完整空间

![洞天](assets/cave-heaven-datacenter.png)

*万颗灵核排列成阵，灵池碧光幽幽，经脉如蛛网般连接一切。这就是洞天——修炼神兽的完整空间。*

灵气（电力）→ 灵核（芯片）→ 灵池（HBM 显存）→ 经脉（互联网络）→ 驭核术（CUDA/XLA）→ 洞天（数据中心）。六层架构，缺一不可。
...
| [人物谱](outline/characters.md) | 真名 ↔ 道号 ↔ 门派 ↔ 故事 | 80+ 人物 |
| [风格指南](outline/style-guide.md) | 术语映射、智元/神兽/洞天体系、写作规则 | 100+ 术语 |
...
- [x] 世界观设定（智元/神兽/洞天体系、100+ 术语映射）
- [x] 时间线梳理（互联网前史 → 10 个纪元 → 未来）
- [x] 人物谱（80+ 人物，含六小龙/大厂军团/西方势力/互联网先驱）
- [x] 御兽之法详解（REINFORCE → PPO → DPO → GRPO → DAPO → veRL）
- [x] 合阵修炼详解（DP/TP/PP/EP/SP/CP 六维并行）
- [x] 总纲定稿（七篇三十二章 + 进度追踪）
- [ ] 素材补全（24/32 章素材就绪，75%）
- [ ] 正文写作（0/32 章）
